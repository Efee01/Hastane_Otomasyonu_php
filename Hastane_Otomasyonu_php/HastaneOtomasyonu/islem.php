...
//veritabanı ekleme kısımı
    $sorgu=$db->prepare('INSERT INTO kullanici SET kullanici_tc=?,kullanici_adsoyad=?,kullanici_password=?');
...
if(isset($_POST['randevu_kaydet'])){
    ...
    $kaydet=$db->prepare("INSERT INTO randevu SET randevu_sehir=?, randevu_hastane=?,
    randevu_doktor=?,
    randevu_tarih=?,
    randevu_klinik=?,
    randevu_hasta_id=?
    ...
    if($insert){
        ...
        header("location:anasayfa.php?kayitbasarisizoldu");
    }
